...
class JsonDB {
  ...
  public getDistricts(): District[] { return this.data.districts; }
  public getVillages(): Village[] { return this.data.villages; }
  public getShelters(): Shelter[] { return this.data.shelters; }
  public getRoads(): Road[] { return this.data.roads; }
  public getAlerts(): Alert[] { return this.data.alerts; }
  public getHouseholds(): Household[] { return this.data.households; }
  public getFieldReports(): FieldReport[] { return this.data.fieldReports; }
  public getIncidents(): Incident[] { return this.data.incidents; }
  public getDataSources(): DataSource[] { return this.data.dataSources; }

  // Updaters
  public updateVillage(id: string, updates: Partial<Village>): Village | null {
    const idx = this.data.villages.findIndex(v => v.id === id);
    if (idx === -1) return null;
    this.data.villages[idx] = { ...this.data.villages[idx], ...updates } as Village;
    this.save();
  ...
  public updateShelter(id: string, updates: Partial<Shelter>): Shelter | null {
    ...
    if (idx === -1) return null;
    this.data.shelters[idx] = { ...this.data.shelters[idx], ...updates } as Shelter;
    this.save();
  ...
  public updateRoad(id: string, updates: Partial<Road>): Road | null {
    ...
    if (idx === -1) return null;
    this.data.roads[idx] = { ...this.data.roads[idx], ...updates } as Road;
    this.save();
  ...
  public updateAlert(id: string, updates: Partial<Alert>): Alert | null {
    ...
    if (idx === -1) return null;
    this.data.alerts[idx] = { ...this.data.alerts[idx], ...updates } as Alert;
    this.save();
    return this.data.alerts[idx];
  }

  public addAlert(alert: Omit<Alert, 'id' | 'timestamp'>): Alert {
  ...
  public updateHousehold(id: string, updates: Partial<Household>): Household | null {
    const idx = this.data.households.findIndex(hh => hh.id === id);
    if (idx === -1) return null;
    this.data.households[idx] = { ...this.data.households[idx], ...updates, lastUpdated: new Date().toISOString() } as Household;
    this.save();
    return this.data.households[idx];
  }

  public addHousehold(hh: Omit<Household, 'id' | 'lastUpdated'>): Household {
  ...
  public addFieldReport(rep: Omit<FieldReport, 'id' | 'timestamp'>): FieldReport {
  ...
  public addIncident(inc: Omit<Incident, 'id' | 'timestamp'>): Incident {
  ...
  public updateDataSource(id: string, updates: Partial<DataSource>): DataSource | null {
    const idx = this.data.dataSources.findIndex(ds => ds.id === id);
    if (idx === -1) return null;
    this.data.dataSources[idx] = { ...this.data.dataSources[idx], ...updates, lastUpdated: new Date().toISOString() } as DataSource;
    this.save();
