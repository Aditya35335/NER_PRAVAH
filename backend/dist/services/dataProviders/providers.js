...
exports.providers = exports.LiveNominatimGeocoding = exports.IntelligentShelterRouter = exports.AILandslidePredictor = exports.LiveWeatherProvider = void 0;
...
class LiveWeatherProvider {
    async getWeather(lat, lng) {
        const openWeatherKey = apiConfig_1.apiConfig.getKey('WEATHER_API_KEY') || '54aed839fb82bd7c5f6c957ca7365960';
        ...
        try {
            if (openWeatherKey) {
                ...
                if (owRes.ok) {
                    ...
                    const rainNow = owData.rain?.['1h'] ?? 0;
...
class AILandslidePredictor {
    ...
    async predictLandslideRisk(lat, lng, locationName = 'Searched Location') {
        ...
        let fsRisk;
        if (fsParams.FS < 1.0)
            fsRisk = 90 + Math.min(8, (1.0 - fsParams.FS) * 80);
        else if (fsParams.FS < 1.1)
            fsRisk = 75 + (1.1 - fsParams.FS) * 150;
        else if (fsParams.FS < 1.3)
            fsRisk = 55 + (1.3 - fsParams.FS) * 100;
        else if (fsParams.FS < 1.5)
            fsRisk = 30 + (1.5 - fsParams.FS) * 125;
        else
            fsRisk = Math.max(5, 30 - (fsParams.FS - 1.5) * 20);
...
class IntelligentShelterRouter {
    ...
    async findSafestShelterAndRoute(userLat, userLng, targetShelterId) {
        const allShelters = database_1.db.getShelters();
        const allRoads = database_1.db.getRoads();
        ...
        const pts = 24;
        const curvedCoords = Array.from({ length: pts + 1 }, (_, i) => {
            const t = i / pts;
